...
class Site extends CI_Controller {
    ...
    public function createreviewsssubmit() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if ($this->photos_model->create($name, $order, $image, $photoalbum) == 0) $data["alerterror"] = "New photos could not be created.";
            else $data["alertsuccess"] = "photos created Successfully.";
    ...
    function createusersubmit() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if ($this->user_model->create($name, $email, $password, $accesslevel, $status, $socialid, $logintype, $image, $json) == 0) $data['alerterror'] = "New user could not be created.";
    ...
    function editusersubmit() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if ($this->user_model->edit($id, $name, $email, $password, $accesslevel, $status, $socialid, $logintype, $image, $json) == 0) $data['alerterror'] = "User Editing was unsuccesful";
    ...
    public function createeventssubmit() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if ($this->events_model->create($name, $image, $venue, $description, $photoalbum, $videoalbum) == 0) $data["alerterror"] = "New events could not be created.";
            else $data["alertsuccess"] = "events created Successfully.";
    ...
    public function editeventssubmit() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $photoalbum = $this->input->get_post("photoalbum");
            $videoalbum = $this->input->get_post("videoalbum");
            if ($this->events_model->edit($id, $name, $image, $venue, $description, $photoalbum, $videoalbum) == 0) $data["alerterror"] = "New events could not be Updated.";
            else $data["alertsuccess"] = "events Updated Successfully.";
            $data["redirect"] = "site/viewevents";
            $this->load->view("redirect", $data);
        }
    }
 ...
 INNER JOIN `status` ON `reliance_photoalbum`.`status`=`status`.`id`");
    ...
    public function createphotoalbumsubmit() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $order = $this->input->get_post("order");
            $status = $this->input->get_post("status");
            if ($this->photoalbum_model->create($name, $order, $status) == 0) $data["alerterror"] = "New photoalbum could not be created.";
            else $data["alertsuccess"] = "photoalbum created Successfully.";
    ...
    public function editphotoalbumsubmit() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $order = $this->input->get_post("order");
            $status = $this->input->get_post("status");
            if ($this->photoalbum_model->edit($id, $name, $order, $status) == 0) $data["alerterror"] = "New photoalbum could not be Updated.";
            else $data["alertsuccess"] = "photoalbum Updated Successfully.";
            $data["redirect"] = "site/viewphotoalbum";
            $this->load->view("redirect", $data);
        }
    }
    ...
    public function editphotossubmit() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            //$image=$this->input->get_post("image");
            $photoalbum = $this->input->get_post("photoalbum");
            if ($this->photos_model->edit($id, $name, $order, $image, $photoalbum) == 0) $data["alerterror"] = "New photos could not be Updated.";
            else $data["alertsuccess"] = "photos Updated Successfully.";
            $data["redirect"] = "site/viewphotos?id=" . $photoalbum;
            $this->load->view("redirect2", $data);
        }
    }
    ...
    public function createvideoalbumsubmit() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $status = $this->input->get_post("status");
            $name = $this->input->get_post("name");
            if ($this->videoalbum_model->create($order, $status, $name) == 0) $data["alerterror"] = "New videoalbum could not be created.";
            else $data["alertsuccess"] = "videoalbum created Successfully.";
    ...
    public function editvideoalbumsubmit() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $status = $this->input->get_post("status");
            $name = $this->input->get_post("name");
            if ($this->videoalbum_model->edit($id, $order, $status, $name) == 0) $data["alerterror"] = "New videoalbum could not be Updated.";
            else $data["alertsuccess"] = "videoalbum Updated Successfully.";
            $data["redirect"] = "site/viewvideoalbum";
            $this->load->view("redirect", $data);
        }
    }
    ...
    public function createvideossubmit() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if ($this->videos_model->create($name, $order, $photoalbum, $url) == 0) $data["alerterror"] = "New videos could not be created.";
            else $data["alertsuccess"] = "videos created Successfully.";
    ...
    public function editvideossubmit() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $photoalbum = $this->input->get_post("photoalbum");
            $url = $this->input->get_post("url");
            if ($this->videos_model->edit($id, $name, $order, $photoalbum, $url) == 0) $data["alerterror"] = "New videos could not be Updated.";
            else $data["alertsuccess"] = "videos Updated Successfully.";
            $data["redirect"] = "site/viewvideos?id=" . $photoalbum;
            $this->load->view("redirect2", $data);
        }
    }
    ...
    public function createfeedbacksubmit() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if ($this->feedback_model->create($salutation, $firstname, $lastname, $middlename, $email, $contact) == 0) $data["alerterror"] = "New feedback could not be created.";
            else $data["alertsuccess"] = "feedback created Successfully.";
    ...
    public function editfeedbacksubmit() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $email = $this->input->get_post("email");
            $contact = $this->input->get_post("contact");
            if ($this->feedback_model->edit($id, $salutation, $firstname, $lastname, $middlename, $email, $contact) == 0) $data["alerterror"] = "New feedback could not be Updated.";
            else $data["alertsuccess"] = "feedback Updated Successfully.";
            $data["redirect"] = "site/viewfeedback";
            $this->load->view("redirect", $data);
        }
    }
    ...
    public function generateexcel() {
        $this->load->dbutil();
        $query = $this->db->query("SELECT `id`,`name` as `Name`,`email` as `Email`,`phone` as `Phone`,`jerseyscore` as `Jersey Score`,`testtime`,flexibility,lightweight,easytocarry,allfeature,screenclarity,stylus,easytouse,otherfeature,travel,harddrive,alluse,versatile,builtinstylus,otheruse,recommend,updates,city
